	...
	public class CommonDrawing1
	{
		public static void DrawSelection(Graphics g, Vs2010CommonColorTable colorTable, Rectangle rect, bool rounded)
		{
			checked
			{
				...
				using (LinearGradientBrush brush = new LinearGradientBrush(rect2, colorTable.SelectedGradientTop, colorTable.SelectedGradientMiddle, LinearGradientMode.Vertical))
				{
					g.FillRectangle(brush, rect2);
				}
				using (SolidBrush brush2 = new SolidBrush(colorTable.SelectedGradientBottom))
				{
					g.FillRectangle(brush2, rect3);
				}
				using (Pen pen = new Pen(colorTable.SelectedBorder))
				{
					if (rounded)
					{
						CommonDrawing.DrawRoundedRectangle(g, pen, rect.X, rect.Y, rect.Width, rect.Height, 2f);
					}
					else
					{
